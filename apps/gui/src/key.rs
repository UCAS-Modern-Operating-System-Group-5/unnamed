        ...
        for (i, part) in parts.iter().enumerate() {
            ...
            if !is_last {
                match *part {
                    ...
                    "M" | "Meta" | "Cmd" | "Command" | "Super" | "Win" => set_mod!(command),
...
impl KeyShortcut {
    pub fn format_string(&self) -> String {
        ...
        if self.0.modifiers.ctrl { s.push_str("Ctrl-"); }
        if self.0.modifiers.alt { s.push_str("Alt-"); }
        if self.0.modifiers.shift { s.push_str("Shift-"); }
        if self.0.modifiers.command { s.push_str("Cmd-"); }
...
pub fn default_key_config() -> KeyConfig {
    key_config! {
        ...
        Scope::SearchbarMain => {
            "Enter" => UserCommand::NextItem,
            "Esc" => UserCommand::QuitApplication,
        }
    }
}
...
mod test {
    ...
    #[rstest]
    #[case("Down", KeyShortcut(egui::KeyboardShortcut::new(Modifiers::NONE, Key::ArrowDown)))]
    #[case("Alt-Down", KeyShortcut(egui::KeyboardShortcut::new(Modifiers::ALT, Key::ArrowDown)))]
    ...
    #[case("Alt-Down", KeyShortcut(egui::KeyboardShortcut::new(Modifiers::ALT, Key::ArrowDown)))]
    #[case("Ctrl-Alt-A", KeyShortcut(egui::KeyboardShortcut::new(Modifiers::ALT | Modifiers::CTRL, Key::A)))]
    fn test_format_string(#[case] expected_format_string: &str, #[case] key_shortcut: KeyShortcut) {
